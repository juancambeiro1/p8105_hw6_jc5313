...
**Description of raw data:** There are `r nrow(homicide)` observations and `r ncol(homicide)` variables in the dataset.
...
First, I will load and clean the birthweight data and describe the raw data.
```{r}
birthweight =
...
**Description of raw data:** There are `r nrow(birthweight)` observations and `r ncol(birthweight)` variables in the dataset.
...
Next, I will compare my proposed regression model ("model_a") to two others:

* model_b: length at birth and gestational age as predictors (main effects only)
* model_c: head circumference, length, sex, and all interactions (including the three-way interaction) between these
...
I use `crossv_mc` and `map` functions in `purrr` to make a comparison between the cross-validated predicton errors of all 3 models. 
...
**Best model:** It seems like Model C ("model_c"), which is the model that has the interaction terms, has the best fit of the three models since it has the lowest RMSE.